  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const run = (timestamp: number) => {
        ...
        // Skip frames when off-screen or when intensity is effectively zero
        const effectiveIntensity = Math.abs(currentIntensity) < 0.001 ? 0 : currentIntensity
        if (!isVisible && effectiveIntensity < 0.01) {
        ...
        // Skip drawing if intensity is effectively zero (no distortion)
        if (Math.abs(currentIntensity) < 0.001) {
